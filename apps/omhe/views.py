...
@json_login_required
@csrf_exempt
def update_transaction(request, update = True):
    print "This is to update an existing transaction"
    return create_transaction(request, update=True)

@json_login_required
@csrf_exempt
def create_transaction(request, update=False):
    if request.method == 'POST':
        form = LymphomaTransactionForm(request.POST)
        if form.is_valid():
            data = form.cleaned_data
    ...
    #this is an HTTP GET
    return render_to_response('lymphoma/create.html',
        {'form': LymphomaTransactionForm(),}, RequestContext(request))
