...
def get_user_profile_info(identifier):
    ...
    up = None
    try:
        up = UserProfile.objects.get(user__username=identifier)
    except UserProfile.DoesNotExist:
        try:
           up = UserProfile.objects.get(user__email=identifier) 
        except UserProfile.DoesNotExist:
            try:
                up = UserProfile.objects.get(vid=identifier) 
            except UserProfile.DoesNotExist:
                try:
                    up = UserProfile.objects.get(anonymous_patient_id=identifier) 
                except UserProfile.DoesNotExist:
                    up=None
    return up          
